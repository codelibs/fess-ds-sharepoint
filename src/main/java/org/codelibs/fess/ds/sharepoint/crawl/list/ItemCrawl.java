...
public class ItemCrawl extends SharePointCrawl {
    ...
    private String buildContent(final GetListItemValueResponse response) {
        ...
        response.getValues().entrySet().stream()
                ...
                .filter(entry -> (includeFields.size() == 0 || includeFields.contains(entry.getKey())) && !excludeFields.stream().anyMatch(exField -> entry.getKey().matches(exField)))
                .forEach(entry -> {
    ...
    private static final List<String> EXCLUDE_FIELDS = Arrays.asList(
            "odata.*",
            "OData__.*",
            "ContentTypeId",
            "Title",
