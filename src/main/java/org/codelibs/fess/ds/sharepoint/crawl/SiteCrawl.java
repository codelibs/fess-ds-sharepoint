...
public class SiteCrawl extends SharePointCrawl {
    ...
    public SiteCrawl(SharePointClient client, SharePointCrawler.CrawlerConfig config, Map<String, GetListItemRoleResponse.SharePointGroup> sharePointGroupCache) {
        super(client);
        this.config = config;
    ...
    public Map<String, Object> doCrawl(final Queue<SharePointCrawl> crawlingQueue) {
        ...
        GetFoldersResponse getFoldersResponse = client.api().doclib().getFolders().setServerRelativeUrl("/sites/" + config.getSiteName() + "/").execute();
        getFoldersResponse.getFolders().stream()
                .filter(folder -> !isExcludeFolder(folder.getName()))
                .forEach(folder -> {
                    targetFolderName.add(folder.getName());
                    crawlingQueue.offer(new FolderCrawl(client,
                            folder.getServerRelativeUrl(),
                            config.isSkipRole(),
                            sharePointGroupCache));
                });
        GetListsResponse getListsResponse = client.api().list().getLists().execute();
        getListsResponse.getLists().stream()
                ...
                .forEach(list ->
                    crawlingQueue.offer(new ListCrawl(client,
                            list.getId(),
                            list.getListName(),
                            config.getListItemNumPerPages(),
                            sharePointGroupCache,
                            isSubPageList(list.getEntityTypeName()),
                            config.isSkipRole(),
                            config.getListContentIncludeFields(),
                            config.getListContentExcludeFields()))
                );
    ...
    private static final List<String> defaultExcludeListEntityTypes = Arrays.asList(
            "OData__.*",
    ...
    private static final List<String> defaultExcludeFolderTitle = Arrays.asList(
            "IWConvertedForms",
            "_private",
            "Style Library",
            "_catalogs",
